...
function createExcludeList(path: string) {
	...
	dirs.forEach((dir, dirI) => {
		...
		for (let i = 0; i < dir.length; i++) {
			...
			if (dir.slice(0, i).length > 0) excludes.push(`${dirsSoFar}${dir.slice(0, i)}`);
		}
...
function getExcludes() {
	...
	try {
		...
		const inspect = config.inspect<ExcludeObject>('exclude');
		return inspect ? { ...(inspect.workspaceValue || {}) } : {};
	}
	catch (error) {
		vscode.window.showErrorMessage(error.message || error);
	}
